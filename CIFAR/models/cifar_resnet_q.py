...
from quantization import Conv2dQuantized, Pact
...
class Model(nn.Module):
    ...
    class Block(nn.Module):
        ...
        def __init__(self, f_in: int, f_out: int, downsample=False):
            ...
            self.conv1 = Conv2dQuantized(f_in, f_out, kernel_size=3, stride=stride, padding=1, bias=False)
            self.bn1 = nn.BatchNorm2d(f_out)

            self.conv2 = Conv2dQuantized(f_out, f_out, kernel_size=3, stride=1, padding=1, bias=False)
            ...
            if downsample or f_in != f_out:
                self.shortcut = nn.Sequential(
                    Conv2dQuantized(f_in, f_out, kernel_size=1, stride=2, bias=False),
    ...
    def __init__(self, plan, initializer, outputs=None, mask=None):
        super(Model, self).__init__()
        outputs = outputs or 10

        # Initial convolution.
        current_filters = plan[0][0]
        self.conv = Conv2dQuantized(3, current_filters, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, x):
        if _QUANT_ACT:
            out = pact(self.bn(self.conv(x)), self.alpha, K)
        else:
